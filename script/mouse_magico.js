...
//******INICIO DA CONFIGURAÇÃO DO MAUSE MAGICO****
// ============================
// CLIQUE MÁGICO (.GIF + SOM)
// ============================
document.addEventListener('click', function (event) {
    ...
    // Adiciona a classe CSS para o GIF de clique
    gifElement.classList.add('gif-clique-magico');

    // As dimensões podem ser definidas via CSS na classe, mas também podem ser sobreescritas aqui se necessário
    // gifElement.style.width = '150px';
    ...
    gifElement.onload = function () {
        ...
        setTimeout(() => gifElement.remove(), 800); //tempo em que o gif de clique é executado
        // Define um temporizador para remover o elemento GIF do DOM após 800 milissegundos (0.8 segundos).
    };
    ...
    // Assumindo que 'somCliqueMuted' é uma variável global definida em outro lugar (ex: let somCliqueMuted = false;)
    if (!somCliqueMuted) { // Verifica se o som de clique não está mudo
        // Verifica se a variável `somCliqueMuted` é falsa (ou indefinida), indicando que o som não está mudo.
        const somClique = new Audio("sons/CLIQUE.mp3");
        // Cria um novo objeto de áudio com o arquivo de som do clique.
...
function criarTrilhaMagicaDesktop(x, y) {
    ...
    // Adiciona a classe CSS para o GIF de trilha (desktop/mobile)
    gifElement.classList.add('gif-trilha-magica');

    // As dimensões podem ser definidas via CSS na classe, mas também podem ser sobreescritas aqui se necessário
    // gifElement.style.width = '90px';
    // gifElement.style.height = '90px';

    document.body.appendChild(gifElement);
    // Adiciona o GIF ao corpo do documento.

    // Assumindo que 'somArrastarMuted' é uma variável global definida em outro lugar (ex: let somArrastarMuted = false;)
    ...
    gifElement.onload = function () {
        ...
        const offsetY = gifElement.offsetWidth / 2; // Corrigido para usar offsetWidth/2 para manter proporção
...
let ultimoTempoCriacaoGifMobile = 0;
